document.addEventListener('DOMContentLoaded', () => {
  ...
  spacing.textContent += '.user-chip{position:relative;min-height:60px;overflow:hidden!important;gap:8px!important}.user-chip [data-user-email]{flex:1 1 auto!important;min-width:0!important;max-width:100%!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;display:block!important}.user-chip .online-dot{flex:0 0 8px!important}.user-role{position:absolute;left:63px;bottom:9px;margin:0!important;color:#16d3ef;font-size:.62rem;white-space:nowrap;max-width:calc(100% - 75px);overflow:hidden;text-overflow:ellipsis}';
  ...
  const fitUserEmail = () => userEmailElements.forEach((element) => {
    ...
    const available = Math.max(40, chip.clientWidth - 79);
    const fontSize = Math.max(8, Math.min(12, available / Math.max(email.length * 0.68, 1)));
